...
def hashtags_tweet(tweet) :
    """prend en argument un tweet et retourner un dictionnaire de hashtags"""
    hashtags_dict={}
    hashtags = tweet.entities.get("hashtags")
    for hashtag in hashtags :
...
def get_tweets_from_candidates_search_queries(queries,twitter_api=twitter_setup()):
    ...
    for i in range(len(queries)) :
        d[queries[i]] = twitter_api.search(queries[i],language=["french", "english"],rpp=100, count = 100)
    return d
...
def from_dictionnary_to_database(dictionnary,list_key_words):
    ...
    for keyword in list_key_words :
        for tweet in dictionnary[keyword] :
            data_frame = data_frame.append({'User_name': tweet.user.screen_name, 'Nb_followers': tweet.user.followers_count, 'Text': tweet.text, 'Hashtags': hashtags_tweet(tweet), 'RT': tweet.retweet_count, 'Like': tweet.favorite_count, 'Location': tweet.user.location, 'Date': tweet.created_at}, ignore_index=True)
    return data_frame
...
"""df = collect_tweets_from_listkeywords_to_database(['Macron', 'GiletJaune'])
print (df)"""

def collect_replies(tweet_id,user_name):
    reponses_possible=collect_by_keywords("to:"+user_name,nb=100)
    replies=[]
    for reponse in reponses_possible:
        if reponse.in_reply_to_status_id == tweet_id:
            replies.append(reponse)
    return replies

#tweets=collect_by_keywords("Emmanuel Macron",10)
#tweet=tweets[2]
#replies=collect_replies(tweet.id,tweet.user.screen_name)
#if len(replies)!=0:
    #print(collect_replies[0])
